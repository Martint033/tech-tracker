...
class CommuneManager
{
	...
	public function communesParRégion(string $region){
		...
		$sql = 'SELECT nom_region AS region, COM_NOM_MAJ_COURT AS commune_maj_court, COM_NOM_MAJ AS commune_maj, COM_NOM AS commune_nom FROM ' . $this->table  . ' WHERE REG_NOM=' . $region;
